...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    buffLock: Mutex = new Mutex
    prodLock: Mutex = new Mutex
    consLock: Mutex = new Mutex
    buffFull: Semaphore = new Semaphore
    buffEmpty: Semaphore = new Semaphore
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  var
    ...
    monitorLock: Mutex = new Mutex -- Defining monitor lock
  ...
  class ForkMonitor
    ...
    fields
      ...
      starteating : array [5] of Condition -- Signal for eating
    methods
      Init ()
      PickupForks (p: int)
      PutDownForks (p: int)
      Eatingcondition (p: int)
      PrintAllStatus ()
  endClass
  ...
  behavior ForkMonitor
    ...
    method PickupForks (p: int)
      -- This method is called when philosopher 'p' is wants to eat.
        monitorLock.Lock() --lock accuired to pickup fork
        status [p] = HUNGRY -- changing status to hungry
        self.PrintAllStatus ()  -- printing status for the philosopher
        self.Eatingcondition (p) -- check if philosopher can eat
        if status [p] != EATING -- if philosopher can not eat then wait
    ...
    method PutDownForks (p: int)
      -- This method is called when the philosopher 'p' is done eating.
        monitorLock.Lock() --lock accuired to putdown fork
        status [p] = THINKING -- changing status to thinking
        self.PrintAllStatus ()  -- printing status for the philosopher
        self.Eatingcondition((p+1) % 5) -- adjacent neighbor condition
        self.Eatingcondition((p-1) % 5) -- adjacent neighbor condition
        monitorLock.Unlock()
      endMethod

    method Eatingcondition (p: int)
      if status [p] == HUNGRY && status [(p+1) % 5] != EATING && status [(p-1) % 5] != EATING -- checking if both the adjacent neighbors are not eating
        status [p] = EATING -- changing the status
        self.PrintAllStatus () -- printing the status 
        starteating[p].Signal(&monitorLock) -- incase of waiting sending signal to start eating
        endIf
    endMethod 
