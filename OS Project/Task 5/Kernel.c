...
------------------------------ InitFirstProcess ----------------------------------
  ...
  function InitFirstProcess ()
  	--
  	-- This routine will get a new Thread 
  	--
  	   var 
  	     pToThread : ptr to Thread	

  	   pToThread = threadManager.GetANewThread()  -- get a new thread
  	   pToThread.Init("UserProgramThread")		  -- initialize the thread
  	   pToThread.Fork(StartUserProcess, 0)		  -- call to fork
...
------------------------------ StartUserProcess ---------------------------------------
  ...
  function StartUserProcess ()
  	...
  	--
  	  var 
  	  	pToPCB : ptr to ProcessControlBlock 
  	  	pToOpenFile : ptr to OpenFile
  	  	initPC : int
  	  	initUserStackTop : int
  	  	initSystemStackTop : ptr to int
  	  	oldStatus : int

  	  	pToPCB = processManager.GetANewProcess()		-- allocate new PCB with GetANewThread
  	  	pToPCB.myThread = currentThread 			-- Initialize myThread field
  	  	currentThread.myProcess = pToPCB			-- Initialize myProcess field

  	  	pToOpenFile = fileManager.Open("TestProgram1")    -- Open executable file 

  	  	-- create Logical address space and read the executable into it 
  	  	initPC = pToOpenFile.LoadExecutable (&pToPCB.addrSpace)  

  	  	fileManager.Close ( pToOpenFile )			-- close executable file

  	  	-- compute the initial value for userLevel Stack 
  	  	initUserStackTop = ( pToPCB.addrSpace.numberOfPages * PAGE_SIZE )

  	  	-- reset the userLevel Stack top to full size 
  	  	initSystemStackTop = & currentThread.systemStack[SYSTEM_STACK_SIZE-1]

  	  	-- thread into user level
  	  	oldStatus = SetInterruptsTo (DISABLED)		-- disable interrupt
  	  	pToPCB.addrSpace.SetToThisPageTable()		-- Initialize page table 
  	  	currentThread.isUserThread = true 			-- set isUserThread true

  	  	-- call to BecomeUserThread and jump to user level
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  function Handle_Sys_Shutdown ()
      ...
      --FetalError ("System call Handle_Sys_Shutdown invoked")
      nl()
    endFunction
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      --
        ...
        newAddrSpace: AddrSpace = new AddrSpace  -- a local variable of type AddrSpace 
	      ...
	      i = currentThread.myProcess.addrSpace.GetStringFromVirtual (
  	  	...
  	  	pToOpenFile = fileManager.Open(&strBuffer)    -- Open executable file 
  	  	if pToOpenFile == null
          return -1
        endIf
        ...
        newAddrSpace.Init()
  	  	-- create Logical address space and read the executable into it 
  	  	initPC = pToOpenFile.LoadExecutable (&newAddrSpace)
  	  	
         -- free all the frames from previous address space
        frameManager.ReturnAllFrames (&currentThread.myProcess.addrSpace) 
        currentThread.myProcess.addrSpace = newAddrSpace   -- allocate new address space
